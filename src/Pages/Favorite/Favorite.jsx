...
import plus from "../../assets/icon/icons8-minus-96.png";
...
const Favorite = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`https://artogram-server.vercel.app/Favorite?email=${user?.email}`, {
        headers: {
  ...
  const addToSelected = (x) => {
    Swal.fire({
      ...
      confirmButtonText: "Yes , remove from favorite",
    }).then((result) => {
      if (result.isConfirmed) {
        fetch(`https://artogram-server.vercel.app/Favorite/${x}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("ArtAccess")}`,
          },
          ...
          {data.length > 0 ? (
            <>
              <div>
                {data.map((x, i) => (
                  <>
                    <div className="flex lg:flex-row flex-col  gap-x-2 w-9/12 p-3 lg:w-11/12 mx-auto theme-color1 bg-opacity-5 border my-1 rounded-[2rem]">
